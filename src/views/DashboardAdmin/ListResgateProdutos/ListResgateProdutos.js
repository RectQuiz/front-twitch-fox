...
function ListResgateProdutos() {
  ...
  useEffect(() => {
    ...
    dispatch(loadRedeemProducts({ page: 1, last: true }));
  }, [dispatch]);
    ...
    <Content>
      <BackgroundColor>
        {loadingUser ? (
          ...
          <ContentBodyDash>
            <HeaderDashBoard
              reload={() => load_redeem_products(1)}
              title={"Aréa de produtos resgatados"}
              subtitle={"Sua aréa de listagem geral de produtos resgatados."}
            />
            <ContentRowDashBoard>
              {redeemProducts && (
                <ListRedeemProducts
