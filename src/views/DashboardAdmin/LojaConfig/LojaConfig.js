  ...
  useEffect(() => {
    ...
    dispatch(
      loadProducts({
        page: currentPage,
        last: true,
        channel_id: user && user._id ? user._id : null,
      })
    );
    ...
    <Content>
      <BackgroundColor>
        {loadingUser ? (
          ...
          <ContentBodyDash>
            <HeaderDashBoard
              reload={() => load_products(currentPage)}
              title={"Sua aréa de configuração da loja"}
              subtitle={"Aréa de configuração da loja."}
            />
            <ContentRowDashBoard>
              {products && (
                <ItensLastAdd
              ...
              <ContentColumDashBoard
                flex={products && products.length > 0 ? 2 : 1}
              >
                {products && (
                  <ButtonActionProduct
                    iconButton={
                ...
                {products && (
                  <ButtonActionProduct
                    iconButton={
                      <FaSteam size={50} color={colors.primary_dashboard} />
                    }
