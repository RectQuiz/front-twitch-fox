...
import {
  setResponseRedeemPoints,
  setStatusRedeemPoints,
} from "../../../../store/modules/points/actions";
        ...
        <Content>
          ...
          >
            {orderRoleta.map((item) => {
              ...
              return (
                <ContentItemRoleta>
                  <LabelItemRoleta>{item.name}</LabelItemRoleta>
                  <ImageItemRoleta src={count ? Item2Roleta : Item1Roleta} />
                </ContentItemRoleta>
              );
            })}
          </ContentCentroRoleta>
          <ButtonRoleta onClick={girarRoleta} title="TITLE TEXT.">
            <ContentButton
              style={{ animation: ` ${animationButton} 1s infinite alternate` }}
            >
              {!loading ? (
                <ContentImageCanal>
                  <ImageButton src={testePictureRgx(channelSelected)} />
                </ContentImageCanal>
              ) : (
                <ContentLoading>
                  <ScaleLoader
